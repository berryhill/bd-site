...
With a URL input, the command verifies `<title>`, meta description, Open Graph title/description/image, and Twitter card/title/description/image readback, then fetches the advertised image endpoint as Twitterbot, follows redirects, and rejects unreachable or non-image responses. Local HTML-file inputs are metadata-only and cannot validate advertised image reachability or content type.
